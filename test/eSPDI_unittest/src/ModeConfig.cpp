...
{
    if ( SQLITE_OK != sqlite3_open_v2("../../bin/ModeConfig.db", &m_sq3, SQLITE_OPEN_READONLY, NULL ) )
    {
        return;
    }
    ...
    {
        char buff[100];
        snprintf(buff, sizeof(buff), "Select * from [%s]", Table.second.csTableName.c_str());
        csCommand = buff;

        if ( SQLITE_OK != sqlite3_prepare_v2( m_sq3, csCommand.c_str(), EOF, &stmt, NULL ) )
        {
            continue;
        }
        while ( SQLITE_ROW == sqlite3_step( stmt ) )
