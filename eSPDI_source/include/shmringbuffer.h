...
public:
    ...
    T dump_front();
    srb_packet_s  *unparse(int input_id); // dump contents in the buffer to a string
...
template <typename T> inline srb_packet_s *
ShmRingBuffer<T>::unparse(int input_id) {
    assert(_hdr != NULL);
    assert(_v != NULL);
    ...
    ret = (srb_packet_s *)(_v + input_id)->unparse();
