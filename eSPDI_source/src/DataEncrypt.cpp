...
bool CDataEncrypt::Encrypt(const std::string& input, std::string& output)
{
    ...
    {
        outCharVector.push_back('A' + ((outVector[i] >> 4) & 0xf));
        outCharVector.push_back('A' + (outVector[i] & 0xf));
    }
...
bool CDataEncrypt::Encrypt(const std::string& input1, const std::string& input2, std::string& output)
{
    ...
    const std::string& longString = input1.size() > input2.size() ? input1 : input2;
    const std::string& shortString = input1.size() <= input2.size() ? input1 : input2;
    ...
    for (size_t i = 0; i < disorderBuf.size(); ++i)
    {
        if (disorderBuf.at(i) != '\0')
        {
            disorderString += disorderBuf.at(i);
        }
    }
    ...
    char shortStringSizeString[16] = {'\0'};
    sprintf(shortStringSizeString, "%d", shortStringSize);
    disorderString.insert(disorderString.size() / 2, std::string("_") + shortStringSizeString + "_");
...
bool CDataEncrypt::Decrypt(const std::string& input, std::string& output1, std::string& output2)
{
    ...
    int shortSize = atoi(disorderString.substr(firstTagPos + 1, secondTagPos - firstTagPos - 1).c_str());
