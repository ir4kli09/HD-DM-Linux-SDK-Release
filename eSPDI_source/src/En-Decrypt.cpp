...
#ifdef __linux__
#ifdef __WEYE__
#include <../WeyeEnv/buildenv/prebuilts/ndk/current/platforms/android-14/arch-arm/usr/include/asm/io.h>
#include <../WeyeEnv/buildenv/prebuilts/ndk/current/platforms/android-14/arch-arm/usr/include/errno.h>
#elif ANDROID
#include <io.h>
#include "eSPDI.h"
#else
    ...
    if (!file_ptr)
    {
		...
		*/
		...
		for (unsigned long i = 10; i < file_size; i+=1)
		{
			...
			if (fread(&findCell, sizeof(unsigned char), 1, file_ptr) != 1)
			{
				printf("== fread failed\n");
				return EtronDI_EN_DECRYPT_FREAD_FAIL;
			}
			//eyslog("i=%lld %c\n", i, findCell);
